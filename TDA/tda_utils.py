...
#Un-comment the following if needed:

#robjects.r('''
#if (!require("TDA", character.only = TRUE)) {
...
#robjects.r('remotes::install_github("samuelgerber/gmra")')
#robjects.r('remotes::install_github("samuelgerber/mph")')
...
def get_stats(PD,both=False): #Ahmad
    if both:
        PD1, PD0 = PD[1], PD[0]
    else:
        PD1 = PD0 = PD
    lifetime = PD1[:, 1] - PD1[:, 0]
    inf_life = lifetime >= 175
    num_inf = inf_life.sum()

    x = PD1[:, 0] # birth
    y = lifetime # persistence
    
    cluster_mdl = DBSCAN(eps=2.5, min_samples=5).fit(y[~inf_life][:,None])
    labels = cluster_mdl.labels_
    colors = plt.cm.Spectral(np.linspace(0, len(np.unique(labels))))
    color = [colors[label] for label in labels] 
    return inf_life.sum() + (labels == -1).sum(),  np.mean(PD0[:,1] - PD0[:,0]), color, labels 
        
# Visualization functions
def plot_birth_persistence_diagram(bd_mat, inf_val=np.inf, fontsize=None,color='b',clustering=False, **kwargs):
    ...
    if clustering:
        _, _, color, labels = get_stats(bd_mat)    
        for i, label in enumerate(labels):
            plt.text(x[~inf_life][i], y[~inf_life][i], str(label), fontsize=6, ha='right', va='bottom', color='black')   
             
    plt.scatter(x[~inf_life], y[~inf_life], c=color, **kwargs)
    if num_inf > 0:
...
def plot_birth_death_bars(bd_mat, order='birth', inf_val=np.inf, **kwargs):
    ...
    if order == 'birth':
        ...
        raise"Invalid value passed for parameter order. Must be either 'birth' or 'death"
