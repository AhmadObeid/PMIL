...
labelsize = 30
kwargs = {"out_res":224, "max_dist":175, "sigma_factor":8, "nuclei_pts":None, "display_result":False}
...
def get_patch_files_mal(id_):
    ...
    path_mal, dirs, files_mal = next(os.walk( os.path.join(rootDir, 'data', 'patches', 'malignant') ))
    patches_mal = [files_mal[i] for i in range(len(files_mal)) if files_mal[i].startswith(str(id_)+'.')]
    return path_mal, patches_mal


def get_patch_files_ben(id_):
    assert(id_>-1 and id_<717), 'Enter correct id (0-716)'
    path_ben, dirs, files_ben = next(os.walk( os.path.join(rootDir, 'data', 'patches', 'benign') ))
    patches_ben = [files_ben[i] for i in range(len(files_ben)) if files_ben[i].startswith(str(id_)+'.')]
    return path_ben, patches_ben


def get_patch_files_mal_persistence(id_):
    assert(id_>-1 and id_<717), 'Enter correct id (0-716)'
    path_mal, dirs, files_mal = next(os.walk( os.path.join(rootDir, 'data', 'persistence_images', 'malignant') ))
    patches_mal = [files_mal[i] for i in range(len(files_mal)) if files_mal[i].startswith(str(id_)+'.')]
    return path_mal, patches_mal
...
def get_patch_files_ben_persistence(id_):
    assert(id_>-1 and id_<717), 'Enter correct id (0-716)'
    path_ben, dirs, files_ben = next(os.walk( os.path.join(rootDir, 'data', 'persistence_images', 'benign') ))
    patches_ben = [files_ben[i] for i in range(len(files_ben)) if files_ben[i].startswith(str(id_)+'.')]
    return path_ben, patches_ben
...
def compute_nuclei_persistence_image(im_input, out_res=224, max_dist=175, sigma_factor=8,
...
def get_persistence_images(id_):
    ...
    if typeof == label['malignant']:
        ...
        id_files = [files_persistence_mal[i] for i in range(len(files_persistence_mal)) if files_persistence_mal[i].startswith(str(id_)+'.')]
        ...
        id_files = [files_persistence_ben[i] for i in range(len(files_persistence_ben)) if files_persistence_ben[i].startswith(str(id_)+'.')]
        for file_name in id_files:
            #print(file_name)
...
def get_persistence_image(img_path, **kwargs):
    ...
    persistence_img, bd_pairs_mph, dgm_mph = compute_nuclei_persistence_image( im_input=img, **kwargs )
    print(persistence_img.shape)
    return persistence_img, bd_pairs_mph, dgm_mph



def save_persistence(id_):
    ...
    if typeof == label['malignant']:
        ...
        for i in range(len(patches_mal)):
            ...
            if os.path.isfile(file_path_mal) and os.path.isfile(file_path_pd):
                ...
                persistence_img, bd_pairs_mph, dgm_mph =  get_persistence_image(img_path, **kwargs)

                with open(file_path_mal,'wb') as f:
                    pickle.dump(persistence_img, f)

                persistence_diagram = [bd_pairs_mph, dgm_mph]
                with open(file_path_pd,'wb') as f:
        ...
        for i in range(len(patches_ben)):
            ...
            if os.path.isfile(file_path_ben) and os.path.isfile(file_path_pd):
                ...
                with open(file_path_ben,'wb') as f:
                    pickle.dump(persistence_img, f)
