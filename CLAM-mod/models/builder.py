...
from .ahmad_encoders import PIEncoder
import torch
...
def get_encoder(model_name, target_img_size=224,Blacks=True):
    print('loading model checkpoint')
    if model_name == 'resnet50_trunc' or model_name == 'resnet50_trunc_PI':
        model = TimmCNNEncoder(Blacks=Blacks)
    elif model_name == 'resnet50_trunc_3d':
        model = TimmCNNEncoder(Blacks=Blacks,dim=3) #Ahmad
    elif model_name == 'simple_model':
        model = PIEncoder() #Ahmad
